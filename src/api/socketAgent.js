...
export default function actionStartSocket () {
  ...
  this.init = (data) => {
    ...
    this.rendered = true
    /* global  __SOCKETHOST__:false */
    // socket = __SOCKETHOST__.indexOf('ocsscc2') > -1 ? io.connect(`${__SOCKETHOST__}/im/user?id=${data.id}&deviceId=c42f716f-c469-4a2b-a0f0-656b52909c59&token=a878ccd9cb8d171_ HTTP/1.1&channel=web&ip=192.168.232.2&nickname=${data.nickname}&avatar=https://img.awesomes.cn/ts/c6/179864583.jpg&os=Android`, { transports: ['websocket'] }) : io.connect(`${__SOCKETHOST__}/im/user?id=${data.id}&deviceId=43C2F574-616F-4242-B0DA-A&token=54a6ef843792e92_&channel=web&ip=192.168.232.2&nickname=${data.nickname}&avatar=https://img.awesomes.cn/ts/c6/179864583.jpg&os=Android`, { transports: ['websocket'] })
    socket = io.connect(`${__SOCKETHOST__}/im/user?id=${data.id}&deviceId=c42f716f-c469-4a2b-a0f0-656b52909c59&token=a878ccd9cb8d171_ HTTP/1.1&channel=web&ip=192.168.232.2&nickname=${data.nickname}&avatar=https://img.awesomes.cn/ts/c6/179864583.jpg&os=Android`, { transports: ['websocket'] })
    socket.on('reconnect_attempt', () => {
      socket.io.opts.transports = ['polling', 'websocket']
    })
    // a878ccd9cb8d171_ HTTP/1.1
    // ocsscc2
    socket._newChat = true
    socket.on('connect', () => {
      console.log(data, '连接成功')
      let MessageList = {}
      void (store.getters.curUser ? store.getters.curUser.contextList.forEach((e, num) => {
      ...
      .on('response', function (data) { // 用户离线事件
        ...
        void (data.status === 'error' ? store.commit('SET_SOCKET_STATUS', { message: ((data.msg || data.workTimeMsg) ? (data.msg || data.workTimeMsg) : socketMsg[data.code]), show: true }) : '')
        void (data.connectSuccessMsg ? store.commit('SET_AGENT', {
          tenantId: data.tenantId,
          contextId: data.contextId,
        ...
        void (data.chatType === 'CHATBOT' && data.contextId ? store.commit('SET_AGENT', {
          tenantId: data.tenantId,
          contextId: data.contextId,
          status: (data.chatType === 'CHATBOT' ? 0 : 1)
        }) : '')
        let MessageList = store.getters.messageUpdate
        void (!MessageList[data.contextId] ? MessageList[data.contextId] = 0 : '')
        store.commit('SET_MESSAGE_UPDATE', MessageList)
        if (store.getters.curPath !== '/chatPanel' || data.contextId !== store.getters.curAgent.contextId) {
          ...
          void (store.getters.curPath === '/chatList' ? getContext(store.getters.curUser.userId).then(res => {
            if (res.object.length > 0) {
              ...
              ctxtList.forEach((e) => {
                void (e.status === 0 ? e.time = store.getters.curUser.contextList.filter((item, i) => {
        ...
        store.commit('SET_MESSAGE_UPDATE', MessageList)
        void (data.direction === 'OUT' && windowBlur ? newMsgCount() : '')
        if (store.getters.curAgent.contextId === data.contextId || store.getters.curAgent.tenantId === data.tenantId) {
          ...
          void (data.chatType === 'CHATBOT' ? store.commit('APPEND_LOCAL_MESSAGE', data) : '')
        }
    ...
    socket.chatbot = (data) => {
      ...
      socket.emit('chatbot', obj, (data) => {
        void (data.status === 'error' ? store.commit('SET_SOCKET_STATUS', { message: (data.msg ? data.msg : socketMsg[data.code]), show: true }) : '')
